...
export default function Interviews() {
  ...
  const interviews = data.interviews
    .filter((i) =>
      provider
        ? data.jobs.some((j) => j.id === i.jobId && j.companyId === user.companyId)
        : i.userId === user.id,
    )
    .map((i) => ({
      ...i,
  ...
  return (
    <>
      <PageHeader
        ...
        action={
          provider && (
            <ButtonLink to="/provider/applicants">Schedule with a candidate</ButtonLink>
          )
        }
      />
          ...
          {interviews.map((i) => {
            ...
            return (
              <Card className="interview-card" key={i.id}>
                ...
                {i.location && (
                  <p>
                    <MapPin size={13} style={{ display: 'inline' }} /> {i.location}
                  </p>
                )}
                <p>{i.notes}</p>
                {i.link && (
