...
export function readStore() {
  ...
  try {
    memory = JSON.parse(localStorage.getItem(key)) || structuredClone(initialData);
  } catch {
...
export function writeStore(data) {
  try {
    ...
    throw new Error(
      'Browser storage is full or unavailable. Your change could not be saved.',
    );
  }
  memory = data;
  window.dispatchEvent(new Event('interntrack:change'));
  return data;
}
...
export function notify(data, userId, title, message, type) {
